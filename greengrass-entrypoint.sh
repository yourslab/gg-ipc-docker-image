...
#Default options
OPTIONS="-Droot=${GGC_ROOT_PATH} -Dlog.store=FILE -jar ${INIT_JAR_PATH}/lib/Greengrass.jar --provision ${PROVISION} --deploy-dev-tools ${DEPLOY_DEV_TOOLS} --aws-region ${AWS_REGION} --setup-system-service true"
...
parse_options() {
	...
	if [ ${PROVISION} == "true" ]; then
		...
		if [ ${THING_NAME} != default_thing_name ]; then
		    ...
		    OPTIONS="${OPTIONS} --thing-name ${THING_NAME}-${RAND_STRING}"
...
#exec /greengrass/v2/bin/greengrass-cli deployment create   --recipeDir /root/greengrass-components/uat/recipes   --artifactDir /root/greengrass-components/uat/artifacts   --merge "com.example.HelloWorld=1.0.0" > /proc/1/fd/1
